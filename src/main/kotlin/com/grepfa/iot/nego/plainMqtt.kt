...
import java.util.*
data class GrepfaMqttConnectionOptions(
    val broker: String,
    val clientId : String,
    val qos: Int
)

class GrepfaMqttNegotiator(val opt: ChirpStackConnectionOptions) {
    ...
    init {
        try {
            logger.info("start connecting chirpstack mqtt server...")
...
//            exitProcess(1)
    ...
    inner class SubCbUp : SubCb<EvUp>() {
        private val typeReference = object : TypeReference<EvUp>() {}
        override fun type(): TypeReference<EvUp> {
